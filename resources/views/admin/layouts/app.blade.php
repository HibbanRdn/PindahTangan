  ...
  <aside class="sidebar" id="sidebar">
    ...
    <nav class="sidebar-nav">
      ...
      <a href="{{ route('admin.testimoni.index') }}"
         class="nav-item {{ request()->routeIs('admin.testimoni.*') ? 'active' : '' }}">
        ...
        Testimoni
          @php $pendingCount = \App\Models\Testimonial::where('status','pending')->count(); @endphp
          @if($pendingCount > 0)
            <span style="margin-left:auto;min-width:18px;height:18px;padding:0 5px;background:#F59E0B;color:#fff;font-size:10px;font-weight:700;border-radius:20px;display:flex;align-items:center;justify-content:center;">
              {{ $pendingCount }}
            </span>
          @endif
      </a>
